...
@interface SingleLineGraph()<UIScrollViewDelegate>
...
- (void)drawGraph{
    /*
     ******界面布局******
     y轴和y轴刻度值在self上，覆盖在backgroundScrollView上面，这样在backgroundScrollView左右滑动时y轴刻度值仍会显示
     ...
     ******view排列关系******
     ...
     backgroundScrollView(左小半部k_graphLeftMargin范围被yAxisView覆盖)
     )
     self竖直方向：
     ...
     曲线和各刻度线表格
     x轴
     空白 k_graphVerticalMargin（之所以x轴和刻度值之间留出空白，因为原点的x刻度值显示在正下方，y刻度值显示在正左方，y刻度值下面才显示x刻度值）
    ...
    [self calculateYAxis];
    originalPoint = CGPointMake([self xPositionOfAxis:0], ((NSNumber *)self.positionYOfYAxisValues.firstObject).floatValue);
...
/*水平方向
 ...
 k_graphLeftMargin, 坐标轴, k_graphRightMargin
 */
- (CGFloat)widthGraph{
    return self.frame.size.width - k_graphLeftMargin - k_graphRightMargin;
}

-(CGFloat)widthXAxis{
    //x轴的长度，不包括左右的margin
    return positionStepX * (self.xAxisArray.count <= 1 ? 1 : self.xAxisArray.count - 1);
}

/*竖直方向
 self
 backgroundScrollView 高度同 graphBackgroundView
 k_graphVerticalMargin
 曲线坐标轴
 k_graphVerticalMargin
 k_xAxisLabelHeight
 legendView
...
-(CGFloat)heightLegend{
    return self.showLegend ? [LegendView getLegendHeightWithLegendArray:self.legendArray legendType:self.legendViewType withFont:self.textFont width:self.frame.size.width - 2 * SIDE_PADDING] : 0;
}
...
-(CGFloat)heightYAxis{
    return [self heightGraph] - k_graphVerticalMargin - k_graphVerticalMargin - k_xAxisLabelHeight;
}

#pragma mark - 计算各种x轴和y轴的各种长度
-(void)calculatePositionStepX{
    if (self.xAxisArray.count <= 1) {
        //没有点或者只有一个点时，positionStepX等于整个区域宽度
        positionStepX = [self widthGraph];
    }
    else{
...
-(void)calculatePointRadius{
    ...
    if (self.pointRadius > lineWidth) {
        ...
        if (widthThreshold < positionStepX && positionStepX < widthThreshold * 2) {
            self.pointRadius = (self.pointRadius - lineWidth) * (positionStepX - widthThreshold) / widthThreshold + lineWidth;
        }
        else if(positionStepX <= widthThreshold){
...
- (void)calculateYAxis{
    ...
    const CGFloat positionYBottom = k_graphVerticalMargin + [self heightYAxis];
    ...
    if (allPointsSet.count == 0) {
        ...
        positionStepY = (positionYBottom - positionYTop);
    ...
    if (allPointsSet.count <= 2) {
        ...
        [self.positionYOfYAxisValues addObject:@(positionYBottom)];//x轴的位置
        [self.positionYOfYAxisValues addObject:@(positionYBottom - positionStepY)];
        [self.positionYOfYAxisValues addObject:@(positionYTop)];//最高横线位置
    }
    else{
        ...
        if (yCeil == maxY && yFloor == minY) {
            ...
            for (int i = 0; i <= segmentsOfYAxis; ++i) {
                ...
                [self.positionYOfYAxisValues addObject:@(positionYBottom - positionStepY * i)];
            }
        }
            ...
            if (yFloor == minY) {//x轴y刻度值为minY
                ...
                for (int i = 1; i <= segmentsOfYAxis; ++i) {
                    ...
                    [self.positionYOfYAxisValues addObject:@(positionYTop + positionStepY * (segmentsOfYAxis - i))];
                }
            }
                ...
                for (int i = 0; i < segmentsOfYAxis - 1; ++i) {
                    [yAxisValues addObject:@(yFloor + valueStepY * i)];
                    [self.positionYOfYAxisValues addObject:@(positionYFloor - positionStepY * i)];
                }
...
-(void)createGraphBackground{
    //    挪到基类中
    //    handleTapPanLongPress 和 showMakerNearPoint也挪到基类中？
    ...
    [self.backgroundScrollView addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(handleTapPanLongPress:)]];
    if (enablePanAndLongPress) {
        [self.backgroundScrollView addGestureRecognizer:[[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(handleTapPanLongPress:)]];
        //如果UILongPressGestureRecognizer.minimumPressDuration设置过小，则TapGesture也会被认为是LongPressGesture
        [self.backgroundScrollView addGestureRecognizer:[[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(handleTapPanLongPress:)]];
    }
    
    self.graphBackgroundView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, k_graphLeftMargin + [self widthXAxis] + k_graphRightMargin, backgroundScrollView.frame.size.height)];//根据x轴的宽度设置graphBackgroundView的宽度
...
- (void)drawXAxis{
    void(^createXAxisLabel)(NSString *, CGFloat, CGFloat, CGFloat) = ^(NSString *s, CGFloat centerX, CGFloat width, CGFloat top){
        UILabel *l = [[UILabel alloc] initWithFrame:CGRectMake(centerX - width / 2, top, width, k_xAxisLabelHeight)];
    ...
    const CGFloat positionYTop = k_graphVerticalMargin;
    const CGFloat positionYBottom = k_graphVerticalMargin + [self heightYAxis];
    const CGFloat yOfXAxisLabel = positionYBottom + k_graphVerticalMargin;//x轴刻度值label的y位置
    ...
    yAxisView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, k_graphLeftMargin, self.graphBackgroundView.frame.size.height - k_xAxisLabelHeight)];
    ...
    //显示原点的x轴刻度值
    createXAxisLabel(self.xAxisArray[0], x, MIN(k_graphLeftMargin, positionStepX), yOfXAxisLabel);
    ...
    for (int i = 1; i < self.xAxisArray.count; ++i) {
        ...
        if ((i % numberOfLabelsBetweenVisibleX == 0 && self.xAxisArray.count - 1 - i > numberOfLabelsBetweenVisibleX * 0.7)
            ...
            //显示x轴刻度值
            createXAxisLabel(self.xAxisArray[i], x, MIN(positionStepX, k_graphRightMargin), yOfXAxisLabel);
        }
    }
}
...
- (void)drawYAxis{
    void(^createYAxisLabel)(NSString *, CGFloat, CGFloat, CGFloat) = ^(NSString *s, CGFloat right, CGFloat centerY, CGFloat height){
        UILabel *l = [[UILabel alloc] initWithFrame:CGRectMake(right - k_graphLeftMargin, centerY - height / 2, k_graphLeftMargin, height)];
        l.textColor = self.textColor;
        l.font = self.textFont;
        l.text = s;
    ...
    const CGFloat lineEndX = k_graphLeftMargin + [self widthXAxis];
    ...
    if (shouldShowMinYLabel) {
        createYAxisLabel([self formattedStringForNumber:yAxisValues.firstObject], lineStartX, positionYBottom, positionStepY);
    }
    ...
    for (int i =1; i < self.positionYOfYAxisValues.count; ++i) {
        ...
        if (i < self.positionYOfYAxisValues.count - 1 || shouldShowMaxYLabel) {//非最高横线 或者 should显示最高横线刻度值
            createYAxisLabel([self formattedStringForNumber:yAxisValues[i]], lineStartX, positionY, positionStepY);
        }
    }
}
...
-(void)createMarker{
    ...
    if (![self.dataSource respondsToSelector:@selector(markerViewForline:pointIndex:andYValue:)]) {
        ...
        [backgroundScrollView addSubview:self.defaultMarker];
    }
    ...
    [self.backgroundScrollView.layer addSublayer:self.xMarker];
    ...
    [self.backgroundScrollView.layer addSublayer:self.yMarker];
}
...
- (void)createLegend{
    ...
    self.legendView = [[LegendView alloc] initWithFrame:CGRectMake(SIDE_PADDING, BOTTOM(self.graphBackgroundView), WIDTH(self) - 2*SIDE_PADDING, 0)];
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
    if (scrollView == backgroundScrollView) {
        CGFloat comparedX = backgroundScrollView.contentOffset.x + k_graphLeftMargin;//坐标系原点距离左边缘 k_graphLeftMargin
...
- (void)showMakerNearPoint:(CGPoint)pointTouched checkXDistanceOnly:(BOOL)checkXDistanceOnly{
    ...
    CGPoint contentOffset = backgroundScrollView.contentOffset;
    if (closestPoint.x - (closestPoint.x == originalPoint.x ? 0 : self.pointRadius) < originalPoint.x + contentOffset.x) {
        if (closestPoint.x == originalPoint.x){
        }
        //closestPoint左边缘在y轴左侧，需要将backgroundScrollView向右滑动使其完全显示出来，但是第一个点只显示一半
        CGFloat needScroll = (originalPoint.x + contentOffset.x) - (closestPoint.x - (closestPoint.x == originalPoint.x ? 0 : self.pointRadius));
        contentOffset.x -= needScroll;
        [UIView animateWithDuration:0.2 animations:^{
            self.backgroundScrollView.contentOffset = contentOffset;
        }];
    }
    else if(closestPoint.x + (closestPoint.x == originalPoint.x ? 0 : self.pointRadius) > contentOffset.x + backgroundScrollView.frame.size.width){
        //closestPoint在屏幕外右边，右边缘没有显示出来，需要将backgroundScrollView向左滑动使其完全显示出来
        CGFloat needScroll = (closestPoint.x + self.pointRadius) - (contentOffset.x + backgroundScrollView.frame.size.width);
        contentOffset.x += needScroll;
        [UIView animateWithDuration:0.2 animations:^{
            self.backgroundScrollView.contentOffset = contentOffset;
        }];
    ...
    if ([self.dataSource respondsToSelector:@selector(markerViewForline:pointIndex:andYValue:)]) {
        ...
        if (self.customMarkerView != nil) {
            ...
            CGRect pathFrame = self.graphBackgroundView.frame;
            ...
            pathFrame.size.height -= k_graphVerticalMargin * 2;//graphBackgroundView中曲线区域的rect，去掉四周的空白
            ...
            if (closestPoint.x - pathFrame.origin.x >= viewSize.width
                && closestPoint.x - viewSize.width >= originalPoint.x + backgroundScrollView.contentOffset.x) {
                //如果pathFrame中closestPoint左边空间足够 && backgroundScrollView当前滚动后的显示区域仍然足够
        ...
        [self.backgroundScrollView addSubview:self.customMarkerView];
    }
    else{
