...
- (void)didMarkerDismissInTimeLine:(TimeLineGraph *)graph;//marker消失
@end
...
/*股票分时图
 y轴显示三根横线，以 正负 max(fabs(最高价), fabs(最低价)) 作为y轴的最高、最低刻度值，以昨日收盘价作为y轴中间刻度值。如果 fabs(max值/昨日收盘价 - 1) < 0.02（也即涨跌幅<1%），则将max值设为昨日收盘价*1.01。价格取整3位小数，百分比取整百分数的2位小数。
